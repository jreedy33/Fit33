...
## Sprint Daily Mission Unification (2026-04-27)

**Problem.** The Daily Brief shipped earlier on 2026-04-27 fused every integration into one welcome insight, but the Daily Goals slate was selected by an entirely separate path (`get_daily_quests` v3 with 32 params, none of them readiness band or top debt). Result on Joe's 10:30 AM dashboard: brief said "Mid recovery + 100g protein short — eat now and run a lighter session later" while the slate prescribed "exercise_sets_15 — 15 sets in a single workout." The user had to mentally pick which surface to follow. Two independent issues compounded the contradiction: (a) the engine fired the protein debt at 10:30 AM with zero meals logged, treating an absolute deficit as a debt regardless of time-of-day pace; (b) the wearable disconnected mid-day but the cached snapshot kept driving "yellow recovery" copy.

**What shipped (7 phases).**

- **Phase 0 — Engine refinement.** [Fit33/DailyBriefEngine.swift](Fit33/DailyBriefEngine.swift) `nutritionFacet` rewritten to be pace-aware. Linear ramp 7AM→8PM models the typical 25%/35%/40% breakfast/lunch/dinner split. Firing rules: never fire as a debt before 1 PM (handled by the no-breakfast pivot for the zero-meals-logged case at 11 AM–2 PM); 1 PM–6 PM requires `deficit_vs_pace >= 30g`; after 6 PM requires `>= 20g`. Urgency cap dropped from 95 to 75 so a meaningful protein gap never out-votes a real wearable signal except in the evening when the user has actually fallen far behind. Capacity / muscle-debt / hydration urgency caps were rebalanced in the same pass: red capacity is the only signal that pins to 100, green is 75 (out-voted by 6+day muscle debt), yellow is 70, unknown drops to 35; muscle-debt scales `50 + 8/day` capped at 90; hydration caps at 70. New no-breakfast pivot template per goal family in [Fit33/DailyBriefTemplates.swift](Fit33/DailyBriefTemplates.swift) — "Refuel — the day's just getting started." for buildMuscle, "Fuel up — protein keeps cravings down." for loseFat, etc. Stale-wearable invalidation: [Fit33/WhoopService.swift](Fit33/WhoopService.swift) `disconnect()` now chains `await ReadinessService.shared.recompute(force: true)` so the brief's capacity facet flips to `.unknown` immediately when WHOOP's 4d-recover path auto-disconnects — no more stale "Mid recovery" copy on a now-disconnected wearable.

- **Phase 1 — Server: brief signals into quest selection.** New migration [supabase/20260703_get_daily_quests_brief_signals.sql](supabase/20260703_get_daily_quests_brief_signals.sql) extends `get_daily_quests` v3 → v4 (32 → 36 args). Four new tail params: `p_capacity_band` ('green'/'yellow'/'red'/'unknown'), `p_capacity_score`, `p_top_debt_kind`, `p_top_debt_payload JSONB`. All nullable so legacy callers see zero behavior change. **Layer 7 — Capacity Band Re-Ranker** filters hard/very-hard difficulty templates from the eligibility pool on red days, elevates the recovery pool, and forces at least one non-workout slot to be a recovery quest; green days boost PR-flag templates' selection_score by 20%; yellow/unknown no-op. **Layer 8 — Debt Booster** force-elevates one quest from the matching debt category into slot 2 or 3 when the payload meets a "real debt" threshold (protein-vs-pace ≥ 30g, hydration ≥ 500ml, steps gap ≥ 3000). Caps at one debt-driven slot/day so the slate never collapses to all-protein. Response gains `is_brief_influenced BOOLEAN` per quest, computed inline from a local `v_brief_influenced_keys` array — no schema change to `user_daily_quests`. Drops every prior overload via the pg_proc loop (Supabase invariant 12) and post-migration audit confirms exactly one surviving definition (Supabase invariant 28). FE invariant 23 (red recovery override) honored — Layer 7 just makes the slate REFLECT the override decision so users stop seeing 25-set quests on red days.

- **Phase 2 — Client: Decision flows brief→quest.** New `BriefDecision` struct on [Fit33/DailyBriefEngine.swift](Fit33/DailyBriefEngine.swift) carries `capacityBand / capacityScore / topDebtKind / topDebtPayload / goalFamily / boosterChallengeId / linkedQuestKeys` plus a `signature` getter (`<band>|<debt>|<goal>` for analytics grouping). Stamped onto every `DailyBrief`. New `DailyBriefEngine.matchQuests(capacityBand:topDebtKind:slate:)` returns the quest keys on today's slate that match the brief's debt + capacity (1:1 mapping table per `DebtKind` case + recovery pool on red days). Drives the new `.focusQuest(questKey: String)` `BriefCTA` case which scrolls/highlights the matching card instead of jumping tabs. New `preferFocusQuest()` swaps `.openMealLog` / `.logWater` / `.openWeightLog` for `.focusQuest` when a linked quest exists — closes the loop ("100g protein left" → tap → glow on "Hit Protein Goal" card, not a jarring tab change). [Fit33/DailyQuestService.swift](Fit33/DailyQuestService.swift) `gatherUserContext` reads `DailyBriefStore.shared.brief?.decision` and packs the 4 new RPC params; `DailyQuest` decoder picks up `is_brief_influenced` from the v4 response. [Fit33/DailyBriefStore.swift](Fit33/DailyBriefStore.swift) gains `linkedQuestKeys` published separately from the brief so quest cards can highlight without recomposing the brief view tree.

- **Phase 3 — UI: "Today's Mission" framing.** [Fit33/WelcomeBriefRow.swift](Fit33/WelcomeBriefRow.swift) renders a tiny "TODAY'S MISSION" `ds_labelSmall` with 1.2 letter-spacing above the headline so the brief and the goals below visibly belong to one thing. Each `compactQuestRow` in [Fit33/DailyQuestViews.swift](Fit33/DailyQuestViews.swift) gains a "← from brief" pill (`sparkles` icon + "from brief" label, blue accent) inline with the title when `quest.isBriefInfluenced == true` OR the client-side engine matched it (`DailyBriefStore.linkedQuestKeys.contains`). The card stroke gradient flips to blue→cyan for influenced quests; brief tap with `.focusQuest` CTA writes to `DailyBriefStore.pendingQuestFocus`, the wrapper's `.onChange` glow-rings the matching card for 1.2s. "Daily Goals Insights" navigation title in [Fit33/QuestInsightsView.swift](Fit33/QuestInsightsView.swift) renamed to "Why these goals?" — matches the new Mission framing where the sheet answers "why is this on my slate?" rather than just showing 28-day stats.

- **Phase 4 — Mid-day re-rank on band flip.** [Fit33/ReadinessService.swift](Fit33/ReadinessService.swift) captures the previous band before overwriting `todayReadiness`, detects flip + has-wearable-signal + once-per-band-per-day stamp (persisted in `UserDefaults` under `fit33.readiness.bandFlipStamp.v1`), then fires `DailyQuestService.fetchDailyQuests(force: true)` so a red→green transition mid-session swaps recovery quests for PR quests (and vice versa). Existing in-progress quests are preserved by the server's "skip slate generation when v_quest_count > 0" guard — only the SLATE SELECTION run on the new band's signals is forced, not the row-level state.

- **Phase 5 — Readiness drill-down.** New file [Fit33/ReadinessDrillDownSheet.swift](Fit33/ReadinessDrillDownSheet.swift) (~270 lines) is the canonical "why is today red/yellow/green?" surface. Big band hero pill + per-signal breakdown from `DailyReadinessSnapshot.signals` (HRV delta, sleep hours, sleep debt, RHR trend, strain prev, each colored by `Severity`) + "Mission steps from this band" section listing the brief-influenced quests + plain-English coaching copy. Presented from `DashboardWelcomeBriefWrapper` via `.sheet(isPresented:)` rather than via NavigationPath — sheets MUST own their presentation context (PE invariant 6 — `SimpleMealPlanView` taught us this when nested NavigationStacks bounced the meal-log push back to root).

- **Phase 6 — Provenance + measurement loop.** New migration [supabase/20260705_daily_brief_quest_completion_link.sql](supabase/20260705_daily_brief_quest_completion_link.sql) (filename uses `20260705` because `20260704` was already taken by `20260704_dedupe_exercise_performance_history.sql` — actual feature shipped same day). Adds `completed_quest_keys TEXT[]`, `completed_at TIMESTAMPTZ`, and `decision_signature TEXT` to `daily_brief_impressions`. New `append_brief_completed_quest(impression_id, quest_key)` SECURITY DEFINER RPC, IDOR-guarded by checking the impression owner against `auth.uid()`, idempotent (returns `already_logged=true` on re-append). iOS hook lives in `DailyBriefStore` — Combine sub on `DailyQuestService.$quests` calls `BriefTelemetry.logQuestCompletion(impressionId:questKey:)` whenever a quest in `linkedQuestKeys` ticks complete. Closes the measurement loop: "did this brief actually drive a conversion?" is now a SQL query, not a guess.

- **Phase 7 — V2 best-time insight feeds quest selection.** [Fit33/DailyQuestService.swift](Fit33/DailyQuestService.swift) `gatherUserContext` checks for a fresh V2 `bestWorkoutTime` insight (titles `"You crush mornings."` / `"Afternoon is your sweet spot."` / `"Evening lifts hit hardest."`) and overrides the legacy `UserBehaviorLearningEngine.preferredTimeOfDay`. Significance-gated by `V2Analyzer.bestWorkoutTime()` (Pearson n≥12, Δ≥15%, p≤0.15). Existing `p_preferred_time` RPC param — better-sourced.

**Validation.** Extended [Fit33Tests/DailyBriefTests.swift](Fit33Tests/DailyBriefTests.swift) with fixtures for `BriefDecision` Codable round-trip + signature shape, `BriefCTA.focusQuest` round-trip + empty-payload fallback, no-breakfast pivot template (must NOT cite "100g short" — tests the false-alarm fix directly), and `linkedQuestTitle` interpolation token. End-to-end fixtures + capacity matrix tests from the original Daily Brief sprint stay in place.

**The screenshot moment.** After this sprint lands, Joe's 10:30 AM dashboard with yellow WHOOP + 5-day chest debt + 0 meals logged renders **"Yellow recovery — chest & triceps are 5 days overdue. Push day in ~28 min. Eat after to bank gains."** instead of the false-alarm protein headline. The slate gets `complete_workout` (slot 1, anchored) + a non-redundant secondary + `evening_wind_down` if it's late. If WHOOP is connected and recovers from red→green at noon, the slate re-shuffles once to swap the recovery quest for an `exercise_sets_25` PR-flag quest. Tap the brief, the matching card glows. Everything is one Mission.

**What didn't ship.** (1) Brief copy doesn't yet INTERPOLATE the linked quest title into the body line ("Eat now (Crush Protein) and run a lighter session"). The token plumbing (`{linkedQuestTitle}` / `{ifLinked}`) is in `DailyBriefTemplates.swift` and is fed from `DailyBriefEngine.compose`, but no template currently USES it — defer to a templates-only PR so Fitness + Design can write the variants without an engine touch. (2) Scroll-to-quest on `.focusQuest` CTA is currently glow-only (no programmatic ScrollViewReader scroll). Quests usually live just below the welcome card so they're already on-screen in 99% of cases; the glow ring is enough.

---

## Sprint Daily Brief (2026-04-27)

**Problem.** The dashboard welcome card had a single-line "motivational message" subtitle bound to `AdvancedIntelligenceService.getPersonalizedRecommendation` — a 10-priority cascade that picked ONE signal (e.g. "Good day for core & cardio — keep the engine running") and surfaced one sentence. With WHOOP / Oura / Fitbit / Strava / HealthKit / hydration / weight / quests / challenges / programs all wired up, that one sentence wasn't using 90% of the available signal. User feedback (2026-04-27): "I want my app to be the place that user can take all their activities, insights, info overload, and understand a clear actionable step as to what's next to their goal." Canonical north-star example the user described: *"your strain is great for a much needed chest and tricep workout!"* — a fusion of WHOOP capacity + muscle-group recency + Build Muscle goal + actionable verb.

**Architecture pivot.** Stop selecting one priority out of ten. **Compose** instead: every facet emits a `FacetSignal { urgency, trace, payload }` — capacity, debt, goal, booster — and the engine picks the highest-urgency Capacity (always present) plus the highest-urgency Debt that's *compatible* with it (red recovery vetoes "go do legs"; green vetoes "rest day"; already-trained vetoes "do another workout"). Goal shapes verbs ("build" → muscle group; "lose" → Z2 cardio; "endurance" → conditioning circuit). Booster appends "wins your daily quest + your 1v1 with Paul" when the same chosen action also closes a quest gap or a 1v1 gap.

**What shipped.** Five new client files + one migration + one feature flag flip + V2 correlation detectors:

- **`Fit33/DailyBriefEngine.swift`** — `@MainActor` engine with 9 facet helpers (capacity, muscleDebt, nutrition, hydration, stepsGap, recovery, noWorkoutYet, streakRisk, competitionBooster) running in parallel via `async let`, plus the `compatibleDebts(_:band:)` veto filter. Capacity reads `ReadinessService.todayReadiness` + raw `WhoopService.todayStrain.strain` headroom (vs typical 14-strain ceiling); falls back to Oura readiness, Fitbit very-active minutes, then HealthKit active minutes. Muscle debt delegates to `WorkoutSuggestionEngine.getMuscleRecoveryStatesAsync()` (off-main-thread bgContext) and intersects with the `User.fitnessGoal`-preferred split family — so a Build Muscle user who hasn't done chest in 5 days gets "chest & triceps overdue" before a Lose Fat user who'd see "Z2 walk closes the day" instead. Protein deficit uses the same `weight × 0.8` formula every other surface uses (`WorkoutProgressView` / `DashboardView+Macros` / `MealPlanView` canon — never invent a new one). Hydration is *time-of-day-aware* — at 8 AM, 800ml behind expected pace doesn't fire; at 6 PM the same gap does. Streak risk only fires after 4 PM local for users with ≥3-day streaks who haven't trained yet today. `BriefCTACoder` codes `BriefCTA` (associated-value enum) into a code+payload pair so `DailyBrief` stays Codable for the disk cache without a hand-rolled implementation.

- **`Fit33/DailyBriefTemplates.swift`** — pure-data template catalog keyed by `(CapacityBand, DebtKind, GoalFamily)` with `{tokens}` interpolated by the engine (`{muscles}`, `{days}`, `{splitTitle}`, `{eta}`, `{deficitG}`, `{deficitL}`, `{gap}`, `{streak}`, `{booster}`). Booster is intentionally pre-prefixed with " + " inside the interpolator so templates can write `"...wins your daily quest{booster}"` and naturally read either "wins your daily quest" or "wins your daily quest + your 1v1 with Paul" without per-template branching. Catalog covers 30+ canonical compositions including the North-Star (`green × muscleGroup × buildMuscle`), no-wearable degradation (`unknown × *` — never cites recovery / strain), red-day overrides (always routes to recovery), and streak fallbacks for the `.allClear` debt. Falls back through `(band, debt, generalFitness)` → streak template before hitting the generic last-resort.

- **`Fit33/DailyBriefStore.swift`** — `@MainActor ObservableObject` that owns today's brief and recomposes on a 30s throttle when *any* source service updates. Combine subscribes to `ReadinessService.$todayReadiness`, `DailyQuestService.$quests`, `MealService.$todaysMeals`, `HydrationService.$todaySummary`, `WhoopService.$todayRecovery`, `OuraService.$todayReadiness` and routes them all through one debounced `PassthroughSubject` so a burst of changes produces ONE recompute. Disk cache `fit33.dailyBrief.v1` is same-day-only (mirrors `RecommendationCache` shape) so cold launch paints the last brief instantly with zero flicker. Covers PE invariant 25b "recompose on `completedCount` change" automatically because the quest sub fires.

- **`Fit33/WelcomeBriefRow.swift`** — pure presentation row + `DashboardWelcomeBriefWrapper` (PE invariant 9 widget isolation; owns the `@StateObject` so quest/wearable updates don't recompute the whole `DashboardView`). Layout is 4 lines: headline (`subheadline.semibold.primary`), body + chevron (`footnote.medium.secondary`), chip strip (up to 3 `BriefChipPayload`s — capacity / sleep / streak / one debt-specific chip), italic correlation sub-line (V2 only). `DashboardBriefRouter.route(cta:navigationPath:workoutManager:)` translates `BriefCTA` to existing dashboard destinations — `startAutoWorkout` / `startRecoveryDay` flip `WorkoutManager.shouldNavigateToAutoGen = true` (same path the Auto Workout button takes, so FE invariant 23 red-recovery override cascades through auto-gen automatically); `openMealLog` / `logWater` / `openWeightLog` all append `DashboardRoute.mealPlan` since hydration + weight live in the meals/macros surface today; `openChallenge(id)` appends the matching `ActiveChallenge` from `ChallengeService.activeChallenges`. `BriefTelemetry.logImpression` / `logTap` write to `daily_brief_impressions` / `daily_brief_taps` (auth-guarded; silent no-op when offline).

- **`Fit33/V2Analyzer.swift`** + edits to `Fit33/PersonalizedInsightsService.swift` — three of four V2 correlation detectors now have *real* significance-gated Pearson implementations replacing the synthetic stubs that had been gated behind `personalizedInsightsV2 = false` since Sprint 3 (Q2-40):
  - `bestWorkoutTime()` reads up to 200 completed Core Data `Workout`s from the last 60 days (off-main bgContext), computes per-session volume = `Σ set.weight × set.reps` for completed sets, buckets by morning/afternoon/evening, requires `n ≥ 12` and ≥2 slots populated, surfaces only when the best slot is `≥15%` above the mean. Cheap p-value via t-stat on n-2 dof + normal CDF (Abramowitz approximation) so we don't lie about confidence.
  - `proteinNextDayVolume(data:)` pairs `daily_summaries[i].protein` with `daily_summaries[i+1].workoutCount`, requires `n ≥ 10` matched-day pairs, `r ≥ 0.3`, `p ≤ 0.15`, AND a top-tertile vs bottom-tertile uplift `≥ 10%` before writing.
  - `challengeBoostedFrequency(data:)` is `@MainActor` (reads `ChallengeService.shared.activeChallenges`), compares last-30d workout frequency against the prior 60d window when the user has at least one active 1v1 / community challenge today, requires both buckets `≥ 7 days` and an uplift `≥ 30%`.
  - All three upsert via the new `upsertCorrelationInsight(...)` helper which respects Data invariant 36 (`onConflict: "user_id,insight_key"`) and writes the full schema `correlation_type` / `r_squared` / `p_value` / `sample_size` / `wearable_source` from migration 20260507. Legacy `analyzeHydrationPerformanceCorrelation` still has its `volumeValues = []` placeholder and harmlessly returns r=0 (never surfaces coaching) pending a per-day volume column on `daily_summaries`.
  - `AppConfig.FeatureFlags.personalizedInsightsV2 = true` flipped in the same PR. Comment block updated to reflect "3 of 4 detectors are now real" and that the hydration stub stays r=0 until the volume column lands.

- **`supabase/20260701_daily_brief_telemetry.sql`** — `daily_brief_impressions (user_id, capacity_band, capacity_source, debt_kind, goal_family, has_booster, cta_code, source_trace, surfaced_at, client_timezone)` + `daily_brief_taps (impression_id, user_id, tapped_at, completed_action_at)`. RLS pinned to `auth.uid()` for all CRUD; service_role retained for nightly aggregation. Indexes for `(user_id, surfaced_at DESC)` and `(capacity_band, debt_kind, goal_family)` so the analytics query "which template family converts best" is cheap. Hot-path independent — nothing in the app reads from these tables; the iOS uploader is fire-and-forget.

- **`Fit33/DashboardView+Header.swift`** — replaces the single-line `Text(personalizedRecommendation?.message ?? currentMotivationalMessage)` subtitle with `DashboardWelcomeBriefWrapper(navigationPath: $dashboardNavPath)`. Old `personalizedRecommendation` `@State` left alive on `DashboardView` since `BackgroundChallengeSyncService.refreshCachedRecommendation()` still writes to `RecommendationCache` (background callers consume it; no UI). Both can coexist; the legacy path is harmless dead UI state.

- **`Fit33Tests/DailyBriefTests.swift`** — 19 fixture tests covering `GoalFamily(rawGoal:)` mappings, `CapacityBand(readiness:)` (incl. the no-wearable veto), `BriefCTACoder` round-trips (auto-workout, challenge, unknown-fallback), the North-Star template (`green × muscleGroup × buildMuscle` with booster — exact-string match on the canonical headline + body), red-recovery override (never suggests "push" on a red day), CTA routing (red → recovery, protein → meal log, hydration → water, muscle → auto-workout with split hint), no-wearable degradation (never cites strain/recovery copy when capacity is unknown), streak fallback for `.allClear`, and `V2Analyzer.pearson` math (perfect positive / negative / zero-variance / mismatched-length).

**Why composition vs cascade.** The legacy 10-priority cascade had a structural problem: each priority returned the moment it fired, so the user never saw a sentence that combined two signals. A Build Muscle user with green WHOOP, chest 5d overdue, AND a 1v1 challenge they were losing would get "Chest, shoulders & triceps are fresh — push day suits you 💪" — perfectly correct, but the 1v1 signal was completely lost. The composition rule lets the SAME user see "Strain is fresh — chest & triceps are 5 days overdue. Push day in ~28 min wins your daily quest + your 1v1 with Paul." All four signals in one sentence, one tap to start the workout that closes all of them.

**Capacity-veto is the discipline.** Without compatibility filtering, the engine would happily produce "Strain is critical — go do chest" on a red day. The `compatibleDebts(_:band:)` filter explicitly drops `muscleGroup` / `stepsBehindGoal` / `streakRisk` debts on red days (only `recoveryNeeded` / `proteinDeficit` / `hydrationDeficit` survive); on green/yellow it drops `recoveryNeeded` so we never contradict a wearable saying the user is primed. FE invariant 23 (red recovery → mobility/walk override in auto-gen) is honored automatically because the CTA layer always routes red days to `WorkoutManager.shouldNavigateToAutoGen = true`, and the auto-gen path applies the override itself; the brief just promises the right destination.

**FE invariant 20a-relative compliance.** The brief CAN reference a 1v1 / community challenge gap as a *Booster* ("...+ your 1v1 with Paul"), but never as the only debt — Booster only surfaces when there's already an independent reason for the user to take an action today (muscle debt, protein deficit, etc.) and the chosen action would also close the challenge gap. The user is never "auto-failed" by another user's behavior because the daily quest slate itself is uncontaminated (FE 20a-relative is enforced at the quest layer, not the brief).

**What didn't ship in this sprint.** (1) A standalone "why is today red?" readiness drill-down sheet — the `BriefCTA.openReadiness` case is a no-op for now, falls back to no navigation. (2) Server-side `get_daily_brief()` RPC — this PR implements client-side composition only; a follow-up could move the fusion into Supabase to avoid duplicating logic across iOS / Watch / web. (3) AdvancedIntelligenceService shim — kept the legacy 10-priority cascade alive as a background-cache warmer for `BackgroundChallengeSyncService.refreshCachedRecommendation()`. The dashboard no longer renders that object, but external callers still do; collapsing to a single composer is a future tidy-up.
...
**Set Initialization Contract** (revised 2026-04-20 — SUPERSEDES 2026-03-17 pre-fill behavior):
- `WorkoutManager.initializeSetsForExercise()` / `initializeSetsForExercises()` create **empty** `WorkoutSetData` rows — they do NOT pre-fill `.weight` or `.reps`.
- Row count = `max(previousSetCount, userDefaultSetCount)` (via `WorkoutManager.previousSetCount(forExerciseId:exerciseName:)`). Example: prev = 3 sets, default = 4 → 4 empty rows, row 4's placeholder falls back to prev set 3 (via `ExerciseCard.getPreviousSetData` last-set fallback).
- Previous weight/reps render as **grey TextField placeholders** (see `SetRowView.weightPlaceholder` and reps placeholder), sourced from `previousExerciseSets[exerciseId]` — NOT from `setData`.
- The "PREVIOUS" / "SUGGESTED" middle column continues to display `weight×reps` from `PreviousSetData`.
- Completing a set without typing (tap ✓) still resolves values from `previousSet` via the SetRowView checkmark fallback chain → actual saved value uses last workout's numbers.
- Do NOT re-introduce `setData.weight = prev.weight` / `setData.reps = prev.reps` anywhere in the init / sync / swap pipeline. That regresses the placeholder UX.
...
- `loadHistoricalDataForExercise()` calls `syncSetsWithPreviousData()` to ADJUST SET COUNT ONLY (row count = `max(previousData.count, defaultSetCount)`). It does NOT pre-fill weight/reps.
- Previous values render as grey placeholders in `SetRowView` via `previousExerciseSets[exerciseId]` (see `DATA_BACKEND_AGENT.md` "Repeat-exercise placeholder contract").
- Will NOT resize rows where user has already entered data (`isCompleted` or non-zero weight/reps).
