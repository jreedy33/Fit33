...
25q-bugintel. **`triage-bugs` Edge Function reads `authoritative_callsite` + `previous_triage` and Claude's `file_path` MUST honor the call-site when present.** `supabase/functions/triage-bugs/index.ts::buildUserPrompt` synthesizes an `authoritative_callsite` object from `last_seen_file:line:function` and a `previous_triage` object from the most recent non-merged `bug_intelligence_reports` row for the same fingerprint. The `SYSTEM_PROMPT` instructs Claude to use the callsite as the definitive `file_path` (`Fit33/<file>`) and align `code_diff` lines within ±5 of `last_seen_line`. NEVER strip `enrichFingerprint`'s `previous_report` fetch — without it Claude re-triages every cluster from scratch and the report list churns ("triage drift"). Same prompt now also includes `severity_score`, the structural classification fields (`op`, `error_class`, `pg_code`, `http_status`, `endpoint`, `structural_fingerprint`), and treats them as ranking inputs.
