-- Fix: v_streak := ROW(0, 0, 0) creates anonymous record without named fields.
-- When user has no streak row, v_streak.current_streak fails with:
--   "record v_streak has no field current_streak"
-- Solution: replace the anonymous ROW() with a SELECT that preserves column aliases.
...
BEGIN
    ...
    IF NOT FOUND THEN
        -- Preserve named fields so v_streak.current_streak etc. resolve correctly
...
GRANT EXECUTE ON FUNCTION get_daily_quests(TEXT, TEXT, BOOLEAN, BOOLEAN, BOOLEAN, INT, TEXT, BOOLEAN, INT, INT, TEXT, INT, BOOLEAN, BOOLEAN, INT) TO authenticated;
