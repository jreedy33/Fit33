...
19b. **Migrations that ALTER columns or CREATE/REPLACE functions MUST end with `NOTIFY pgrst, 'reload schema';`** so PostgREST's API-node cache invalidates immediately instead of waiting for the periodic background refresh (default 10 min on Supabase Cloud, observed lag of 5—12 min during multi-region failover). Without this, every iOS write that touches the new column / new function comes back as `PGRST204` ("Could not find the 'X' column of 'Y' in the schema cache") or `PGRST202` ("Could not find the function ... in the schema cache") for the entire propagation window — manufacturing a per-launch bug-intel fingerprint that goes away on its own and is impossible to repro after the cache catches up. Canonical incident 2026-04-29: migration #164 added `exercise_set_history.exercise_id UUID` without `NOTIFY pgrst` → 6 distinct bug-intel fingerprints (`f5e63f1c`, `c3edd3a3`, `e7e58303`, `91e7baa9`, `0e92e6a9`, `2eb2f0d6`) before the cache reloaded. Defensive remediation in `supabase/20260821_pgrst_schema_reload_audit.sql` (#188) ships a stand-alone `NOTIFY pgrst, 'reload schema';` + audit-block proof that the columns physically exist; reuse the same recipe whenever you need to repair a schema-cache regression mid-day. The `NOTIFY` is a pure no-op when the cache is already current — safe to re-run, safe to over-include, never breaks anything. Ship it as the LAST statement INSIDE the `BEGIN; ... COMMIT;` block (so it fires only on successful commit, not on rollback).
...
### Cardio Redesign Phase 1 — native cardio RPC + columns + quest widening (Sprint 2026-05-02 — migrations #184/#185/#186/#187)
30-cardio. **`supabase/20260814_cardio_native_columns.sql` (#184) ships the native-cardio data shape additions.** Adds `polyline_native TEXT` (Google-encoded polyline of the `simplifiedRouteCoordinates` from `RunningManager`), `splits_native_json JSONB` (per-km/mile splits — array of `{kilometer, time, pace, is_manual}` rows; the daily-quest negative-split detector unions this with the existing `splits_json` for Strava parity), `gps_avg_accuracy_m DOUBLE PRECISION` (rolling avg horizontal accuracy used by the leaderboard junk-run filter), `weather_json JSONB` (best-effort observed weather, future-use). Widens the `goal_type` CHECK to include `'pace'` (matches `RunGoalType.pace.rawKey`). Adds partial index `idx_cardio_workouts_native_recent ON (user_id, completed_at DESC) WHERE origin_app = 'fit33'` for the recap-feed hot path. Idempotent (`ADD COLUMN IF NOT EXISTS`).
30-cardio-2. **`supabase/20260815_record_cardio_workout_rpc.sql` (#185) ships `record_cardio_workout(p_payload JSONB) RETURNS UUID`** (returns the surviving `cardio_workouts.id` — an earlier revision of this doc claimed `RETURNS JSONB` with a rich envelope; that was never true). SECURITY DEFINER, `auth.uid()`-pinned (Data invariant #7 — payload's optional user_id is IGNORED). Behaviors: (a) idempotency on `(user_id, external_id)` — second call with the same external_id returns the existing row; (b) same-origin overlap dedup (existing row whose start..end window overlaps ≥50% on same `origin_app` becomes the survivor); (c) cross-origin Strava merge — when the incoming row is `fit33` and a Strava row covers the same window, the Strava row is kept and the incoming fit33 row is rejected as `merged_into_strava` (returns the Strava row's id); (d) league points: the RPC awards BOTH `add_league_points(...,50,'workout')` AND the graduated `cardio_bonus` (`base + intensity_multiplier * km`, capped +50/day). **KNOWN OPEN BUG (MASTER_TODO PR-14):** `UserManager.completeCardioWorkout` ALSO awards `+50 .workout` client-side and the `workout` source is uncapped → native cardio double-awards. Decision: RPC is the single owner for RPC-backed saves; the client-side award must become conditional; (e) PR detection writes to `cardio_personal_records`; (f) friend-feed insert into `friend_activity_feed` on goal-achieved — the client ALSO posts via `postCardioActivity` (duplicate-feed part of PR-14).
30-cardio-3. **`supabase/20260817_native_cardio_quest_widening.sql` (#186) widens quest verification to native sources.** `is_strava_quest_completed` and `verify_strava_quests_for_today` now both filter `WHERE source IN ('strava', 'fit33')`. Activity-type matching widened: `run_outside_*` accepts both `'outdoor_run'` (Strava) and `'run'` (RunningManager output); `walk_*` accepts `'walk'`; `cycle_outside_*` accepts both `'outdoor_cycle'` and `'outdoor_cycle'` (RunningManager normalizes). Negative-split detection coalesces `splits_json` (Strava) with `splits_native_json` (Fit33) — `COALESCE((splits_json)::jsonb, (splits_native_json)::jsonb)` so either source unblocks the quest. The function name keeps the historical `_strava_` prefix to avoid breaking call sites; the iOS-side wrapper `DailyQuestService.onCardioActivityImported(source:)` is the canonical entry.
30-cardio-4. **`supabase/20260818_cardio_quest_goal_bias.sql` (#187) ships `cardio_goal_bias_score(p_quest_key TEXT, p_fitness_goal TEXT) RETURNS INT`.** IMMUTABLE, returns -50…+30. Bias map covers loseFat (favors calorie burn + Z2 / longer steady-state, mild penalty on sprint), buildMuscle (mild penalty on long-distance, slight favor on Z2 active-recovery), endurance (favors longer distance + tempo + intervals), generalFitness (mild positive bias to active-recovery). Used in the (next-sprint) `get_daily_quests` v5 patch's slot-selection ORDER BY: `... ORDER BY ... + cardio_goal_bias_score(qt.quest_key, up.fitness_goal) DESC` — never as a hard filter (filtering creates "no quests for you" silent failures). Add new cardio quest keys to the function in the same migration as the template insert.
