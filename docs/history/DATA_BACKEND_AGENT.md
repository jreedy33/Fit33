...
## 2026-04-25: Strava Integration Upgrade (Phases 1–5)

**Context**: Pre-upgrade Strava was OAuth-clean but pull-only and "dead UI" — `StravaService.syncActivities` only called `GET /athlete/activities` (list endpoint), `getActivityDetail` and `getAthleteStats` were defined but never invoked, `StravaCompactCard` existed in `StravaSettingsView.swift` but was never referenced from `DashboardView`, and Strava had no widget on the home tab next to WHOOP/Oura. `cardio_workouts` rows landed via `source='strava'` but did not set `origin_app='strava'` and the row payload omitted `suffer_score` even though Strava returns it on the list endpoint. No Strava-specific insight rows in `user_personalized_insights`. No webhook subscription, no server-side token store. Runna investigation: confirmed no public API for third-party developers — option chosen was to treat Runna runs as Strava activities (most users dual-sync), with Apple Calendar / EventKit-based "tomorrow's planned run" hint deferred.

**Phase 1 — Dashboard widget + recap (highest value, cheapest)**:
- New `Fit33/DashboardStravaWidget.swift` (`DashboardStravaWrapper` + `DashboardStravaCard`) modeled on `DashboardWhoopWidget.swift`. Gates on `StravaService.shared.isConnected` + most-recent activity within `now − 36h`. Displays distance, moving time, pace, avg HR, elevation, calories, suffer_score chip when present.
- New `Fit33/StravaActivityRecapSheet.swift` — splits table, HR-zone bar from streams, top 3 segments, polyline mini-map via `MKMapView` decoding the `polyline_summary`.
- `DashboardView` adds `@AppStorage("showStravaWidget") = true` (next to `showWhoopWidget` / `showOuraWidget`) + `WidgetSettingsSheet` row.
- `Fit33/DashboardModels.swift` extends `DashboardNotificationCarousel` with `.stravaRecap(StravaActivity)` case (one-shot per new activity, dismissed-IDs persisted in `@AppStorage("dismissed_strava_recap_ids")`).
- `RunCompletionView` finalize: if `StravaService.shared.isConnected`, fire-and-forget `syncActivities(daysBack: 1)` then attempt to find a Strava activity within ±5 min of run start; if found, call `userManager.completeCardioWorkout(...)` with the Strava-enriched data and `updateStreak()`. Closes the existing gap where GPS runs skipped gamification because recovery happened later via Strava/HealthKit pull.
- Deleted `StravaCompactCard` (dead UI in `StravaSettingsView.swift`) per Phase 1 cleanup.

**Phase 2 — Activity detail enrichment (use the unused endpoints)**:
- Migration `supabase/20260530_cardio_workouts_strava_detail.sql` adds 9 columns: `suffer_score INT`, `kudos_count INT`, `achievement_count INT`, `polyline_summary TEXT`, `splits_json JSONB`, `segment_efforts_json JSONB`, `streams_json JSONB`, `gear_name TEXT`, `detail_synced_at TIMESTAMPTZ`. Partial index `idx_cardio_workouts_strava_pending_enrichment ON (user_id, started_at DESC) WHERE source = 'strava' AND detail_synced_at IS NULL` is the hot-path read for the enricher.
- New `Fit33/StravaActivityEnricher.swift` — sliding-window-throttled enricher (max 5 enrichments per 15-min window via `OSAllocatedUnfairLock`-guarded sliding window) that calls `getActivityDetail(activityId)` + `getActivityStreams(activityId, keys: [heartrate, cadence, watts, velocity_smooth, altitude], key_by_type: true)` and writes all four JSONB columns + the polyline summary + suffer/kudos/achievement scalars in one round-trip. Strava rate-limit envelope is 200/15min and 2000/day — 5 per 15-min per user is the safe budget.
- `StravaService.StravaCardioWorkoutInsert` gains `originApp: "strava"` (closes Data invariant gap noted in `DATA_BACKEND_AGENT.md`) + `sufferScore: Int?`.
- New raw-JSON fetchers `getActivityDetailJSON(id:)` + `getActivityStreamsJSON(id:)` return raw `[String: Any]` so the enricher can serialize directly into JSONB without a typed boundary that would fight Strava's optional-shape responses.

**Phase 3 — Strava insights, streaks, metrics**:
- New edge function `supabase/functions/compute-strava-insights/index.ts` — Deno runtime, mirrors `compute-readiness-insights` shape (auth via `x-cron-key` header OR direct service-role bearer, `EXPECTED_PROJECT_REF` JWT validation). Reads 60d of `cardio_workouts` (source='strava') + `daily_readiness_history` and upserts five insight cards into `user_personalized_insights` with `onConflict: "user_id,insight_key"` (Data invariant #36): `strava_weekly_mileage_delta`, `strava_pace_trend_4w`, `strava_hr_zone_drift`, `strava_segment_pr` (only emitted when a segment effort PR fires from `segment_efforts_json`), `strava_recovery_pairing` (joins WHOOP/Oura readiness with same-day suffer_score for "high-effort runs cost you ~22% next-day recovery"). Significance gate is the existing `p ≤ 0.15, n ≥ 10` filter in `v_user_wearable_insights`.
- Migration `supabase/20260532_strava_insights_cron.sql` schedules `trigger_compute_strava_insights()` at `40 3 * * *` (03:40 UTC) — staggered between `compute-readiness-insights` (03:30) and bug-intel sweeps (03:45 / 04:30) so cold-starts don't pile up. Canonical `internal_config` + `x-cron-key` pattern (Supabase invariant #25 / `trigger_compute_readiness_insights` from migration 72).
- Migration `supabase/20260531_strava_quest_templates.sql` registers three new `quest_templates` rows (`run_outside_3km`, `run_outside_5km`, `cycle_outside_15km`) plus SECURITY DEFINER helper RPC `is_strava_quest_completed(p_user_id UUID, p_quest_key TEXT, p_timezone TEXT)`. RPC pins `auth.uid() = p_user_id` (Data invariant #7), filters `cardio_workouts` on `source IN ('strava')` + `activity_type = ANY(...)` matching `mapStravaActivityType` output (`outdoor_run` / `outdoor_cycle` — treadmill / indoor_cycle EXCLUDED) + `distance_meters >= target` + day-of in caller timezone. Wiring into `get_daily_quests` follows in a small follow-up migration once we've observed the matching pattern in prod.
- `Fit33/StreakShieldService.creditHighEffortDay(reason:)` updates `lastShieldUsedDate` so a tomorrow rest-day cannot break the streak after `suffer_score > 150` activities (does NOT consume a real shield — paint it as a "high-effort streak shield hint"). `StravaService.saveActivitiesToCloud` calls it for any today-activity with `suffer_score > 150`.
- New `Fit33/StravaStatsCharts.swift` — `StravaMileageChartWidget` (weekly mileage stacked by activity type via `BarMark`) + `StravaPaceTrendChartWidget` (4-week rolling avg pace per workout type via `PointMark + LineMark`). Both gate on `StravaService.shared.isConnected` and read directly from `StravaService.shared.recentActivities` (no new RPC, no Core Data round-trip). Wired into `WorkoutStatsView::WorkoutStatsSection`. Local copies of `chartPlaceholder` / `emptyChartState` (renamed `stravaChartPlaceholder` / `stravaEmptyChartState`) to keep the new file self-contained — those helpers are `private` to `WorkoutStatsView.swift`.

**Phase 4 — Cardio section Strava surface**:
- New `Fit33/CardioStravaSection.swift` — `CardioStravaSection` container + `CardioStravaWeeklyDeltaChip` (this week vs last week, distance + time deltas) + `CardioStravaRecentRow` (horizontal scroller of `CardioStravaMiniCard`s). Gated on `StravaService.shared.isConnected`. Mini-cards open `StravaActivityRecapSheet` on tap.
- `CardioLandingView` inserts `CardioStravaSection()` between `headerSection` and `quickStartSection`.

**Phase 5 — Webhooks (instant recaps, server-side tokens)**:
- Migration `supabase/20260533_strava_webhook_tokens.sql` creates `public.user_strava_tokens (user_id PK FK→user_profiles(id) ON DELETE CASCADE, access_token, refresh_token, expires_at, athlete_id, last_rotated_at, timestamps)` with full RLS (SELECT/UPDATE/DELETE pinned to `auth.uid() = user_id`; INSERT is RPC-only). Two RPCs: `upsert_strava_tokens(p_access TEXT, p_refresh TEXT, p_expires_at TIMESTAMPTZ, p_athlete_id BIGINT DEFAULT NULL)` SECURITY DEFINER pinned to `auth.uid()` for the iOS dual-write path (Data invariant #7), AND `get_user_id_for_strava_athlete(p_athlete_id BIGINT)` SECURITY DEFINER service-role-only for the webhook to map Strava `owner_id` → app user_id without exposing the tokens table. `BEFORE UPDATE` trigger maintains `updated_at`.
- `StravaService.mirrorTokensToSupabase(access:refresh:expiresAt:athleteId:)` — best-effort dual-write helper called after `exchangeCodeForTokens` AND after `refreshAccessToken`. Strava rotates the refresh token on every refresh — both the iOS keychain path AND the edge function refresh path persist the new value; `last_rotated_at` distinguishes which side rotated last. Failure never breaks the iOS flow.
- New edge function `supabase/functions/strava-webhook/index.ts` — handles GET `hub.challenge` handshake (verified via `STRAVA_VERIFY_TOKEN` secret) + POST event delivery. ACKs ≤2s (Strava's retry deadline) then processes asynchronously: `get_user_id_for_strava_athlete` lookup → token refresh if needed (5-min skew buffer) → `/activities/{id}?include_all_efforts=true` fetch → `cardio_workouts` upsert keyed on `(user_id, external_id)` with the same Phase 2 column set (origin_app=strava, suffer_score, kudos_count, achievement_count, polyline_summary, splits_json, segment_efforts_json, gear_name, detail_synced_at). Sends `type='strava_activity_new'` silent push only on `aspect_type='create'` (update events are noisy).
- Silent push implementation: direct APNs call mirroring `wake-challenge-opponents` JWT signing — NOT the `push_notification_queue` (that path is for user-visible alerts and `send-push-notification` does not support `content-available: 1`). Required secrets: `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_BUNDLE_ID`, `APNS_PRIVATE_KEY` plus the new `STRAVA_CLIENT_ID`, `STRAVA_CLIENT_SECRET`, `STRAVA_VERIFY_TOKEN`.
- `Fit33/SilentPushHandler.swift` adds `case "strava_activity_new"` → `handleStravaActivityNew(...)` calls `StravaService.shared.syncActivities(daysBack: 1)` + `UserManager.updateStreak()` under the existing 25s self-cap (iOS background task limit).
- Deploy with `--no-verify-jwt` (Strava sends no Supabase auth header). Register in the Edge Function Auth Registry in `INFRA_SECURITY_AGENT.md` per Data invariant #9.

**Run order**: 20260530 (cardio detail columns) → 20260531 (quest templates + helper) → 20260532 (insights cron) → 20260533 (webhook tokens). All standalone, idempotent, additive only. Zero breaking changes to existing `cardio_workouts` consumers.

**Errors caught + fixed during the build**:
1. `is_strava_quest_completed` initially used `cw.workout_type` and raw Strava types (`'Run'`); corrected to `cw.activity_type` with mapped types `outdoor_run` / `outdoor_cycle` (matches what `StravaService.mapStravaActivityType` writes into the table).
2. `StravaStatsCharts.swift` initially called `chartPlaceholder` / `emptyChartState` from `WorkoutStatsView.swift` — those are `private`. Duplicated as `stravaChartPlaceholder` / `stravaEmptyChartState`.
3. Phase 5 silent push initially planned to use `push_notification_queue`. After investigating `wake-challenge-opponents` (the only existing silent push pattern), rewrote to direct APNs JWT-signed call inside the webhook function.

**Pending follow-up**:
- Wire `is_strava_quest_completed` into `get_daily_quests` body once we've validated matching pattern in prod (templates + helper land first; verification wiring follows).
- Apple Calendar / EventKit-based "tomorrow's planned run" Runna hint — deferred (requires permission prompt, low priority).
