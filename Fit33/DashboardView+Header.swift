...
// MARK: - Dashboard Nav Bar Toolbar

/// Leading logo: on iOS 26+ opts out of the Liquid Glass pill via `sharedBackgroundVisibility`.
private struct DashboardNavLeadingToolbar: ViewModifier {
    ...
    private var dashboardNavLogo: some View {
        Image("fit33-logo")
            .resizable()
            .aspectRatio(contentMode: .fill)
            .frame(width: 140, height: 55)
...
/// Trailing controls: on iOS 26+ each item opts out of the shared Liquid Glass pill so they float like the logo.
private struct DashboardNavTrailingToolbar: ViewModifier {
                ...
                .toolbar {
                    ...
                    ToolbarItem(placement: .topBarTrailing) {
                        HStack(spacing: 4) {
                            ...
                            NavigationLink(value: DashboardRoute.profile) {
                                DashboardProfileButton(profilePhotoURL: profilePhotoURL)
                            }
                            .accessibilityLabel("Profile")
                            .accessibilityHint("View your profile and settings")
                        }
...
struct DashboardNavToolbar: ViewModifier {
    @EnvironmentObject var workoutManager: WorkoutManager
    @Binding var showingWidgetSettings: Bool
    let profilePhotoURL: String?
    ...
    func body(content: Content) -> some View {
        content
            .modifier(DashboardNavLeadingToolbar())
            .modifier(DashboardNavTrailingToolbar(
...
private struct DashboardProfileButton: View {
    let profilePhotoURL: String?
