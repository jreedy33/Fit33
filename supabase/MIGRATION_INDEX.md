...
| 77 | `20260511_health_rls_audit.sql` | 🆕 Ready | (Q2-97 Bug-Intel Sweep Cluster B — 2026-04-23) Canonicalizes RLS on the three health-data tables generating the 42501 noise cluster in Bug Intelligence (`cardio_workouts`, `daily_activity_summary`, `healthkit_sleep`). `ALTER TABLE … ENABLE ROW LEVEL SECURITY`, idempotent `DROP POLICY IF EXISTS` sweep, then `CREATE POLICY` for SELECT/INSERT/UPDATE/DELETE using `auth.uid() = user_id`. Trailing `DO $$` block queries `pg_class.relrowsecurity` and `RAISE WARNING`s if any of the three tables still has RLS disabled after commit. Paired Swift: `Fit33/HealthDataService.swift` adds `isAuthenticated` guards on `saveHealthKitWorkout`, `saveSleepFromHealthKit`, `saveDailyActivity`; `Fit33/SupabaseManager.swift` guards `saveStepData` + `batchSaveStepData`. Each guarded early-return logs a `DiagnosticContext(op, endpoint)` at `.info` so the skip is observable without generating fingerprints. |
