-- Fix: "operator does not exist: uuid = text" in get_friend_workout_exercises
-- The RPC compares text parameter p_workout_id against UUID columns (workout_history.id,
-- workout_exercises.workout_id, exercise_performance_history.workout_id).
-- Crash ID: f6d86dd3 (v1.35.0)
...
BEGIN
    -- Cast once, fail gracefully on invalid UUID
    BEGIN
    ...
    WHERE workout_history.id = v_workout_uuid;
    ...
    LEFT JOIN exercise_performance_history p
       ...
       AND p.workout_id = v_workout_uuid
    WHERE e.workout_id = v_workout_uuid
