...
| `wake-challenge-opponents` | User JWT (auth.uid bound) OR service role / `x-cron-key` (for `source: cron` and `source: progress_update`) | Per-source: 15-min for `foreground` / `cron` / `background_sync`; **60s for `progress_update`** (Widget Freshness Phase 7b — server-trigger fan-out, see `supabase/20260625_opponent_progress_silent_push.sql`). Lookup filters `silent_push_wake_log.triggered_by = source` so each bucket throttles independently. | `APNS_*` | Silent push at priority 5. `progress_update` mode requires service-role auth + a `recipient_ids: UUID[]` body — user-JWT calls cannot target arbitrary recipients. |
