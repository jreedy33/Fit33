-- ============================================================================
-- ADD is_verified TO ALL REMAINING SOCIAL RPCs
-- Covers: challenges, community, private challenges, shared workouts,
-- friend requests, search, so the badge shows everywhere.
-- ============================================================================

BEGIN;

-- ============================================================================
-- 1. get_active_challenges — add opponent_is_verified
-- ============================================================================
...
RETURNS TABLE (
    ...
    am_winning BOOLEAN, am_winning_today BOOLEAN, opponent_is_verified BOOLEAN
)
...
-- ============================================================================
-- 2. get_pending_sent_challenges — add opponent_is_verified
-- ============================================================================
...
RETURNS TABLE (
    ...
    start_date TEXT, end_date TEXT, duration_days INT,
    opponent_id UUID, opponent_name TEXT, opponent_username TEXT, opponent_photo_url TEXT,
    sent_at TIMESTAMPTZ, opponent_is_verified BOOLEAN
)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE current_user_uuid UUID;
BEGIN
    ...
    SELECT gc.id, gc.challenge_type, gc.title, gc.description,
        ...
        gc.created_at, COALESCE(up.is_verified, FALSE)
    FROM group_challenges gc
...
-- ============================================================================
-- 3. get_active_group_challenges — add is_verified to members JSON
-- ============================================================================
...
-- ============================================================================
-- 4. get_received_workouts — add sender_is_verified
-- ============================================================================
...
RETURNS TABLE (
    ...
    viewed_at TIMESTAMPTZ, saved_to_favorites BOOLEAN, sender_is_verified BOOLEAN
)
...
-- ============================================================================
-- 5. get_pending_friend_requests — add is_verified
-- ============================================================================
...
RETURNS TABLE (
    ...
    status TEXT, created_at TIMESTAMPTZ, message TEXT, is_verified BOOLEAN
)
LANGUAGE plpgsql SECURITY DEFINER
AS $$
DECLARE current_user_uuid UUID;
BEGIN
    ...
    SELECT f.id, f.requester_id, up.name, up.username, up.profile_photo_url,
        f.status::TEXT, f.created_at, f.message, COALESCE(up.is_verified, FALSE)
    FROM friendships f
...
-- ============================================================================
-- 6. get_sent_friend_requests — add is_verified
-- ============================================================================
...
-- ============================================================================
-- 7. search_users — add is_verified
-- ============================================================================
...
RETURNS TABLE (
    ...
    has_outgoing_request BOOLEAN, has_incoming_request BOOLEAN, is_verified BOOLEAN
)
...
-- ============================================================================
-- 8. Community challenge RPCs — add is_verified to JSON leaderboard entries
-- ============================================================================

-- 8a. get_community_challenge_leaderboard
...
-- 8b. get_my_community_challenges — add is_verified to top_participants JSON
...
-- 8c. get_community_challenge_detail — add is_verified to top_leaderboard JSON
...
-- ============================================================================
-- 9. Private challenge RPCs — add is_verified to member JSON
-- ============================================================================

-- 9a. get_private_challenge_detail
...
-- 9b. get_my_private_challenges — add is_verified to top_members JSON
