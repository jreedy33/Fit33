...
**Run order**: 63 → 64 → 65 (all standalone, idempotent). Safe to re-run. Preserves admin-managed fingerprint fields (`status` / `assigned_agent` / `pain_point_id` / `resolution_pr_url`) across runs.
**Paired code changes**:
- Phase 2: `supabase/functions/triage-bugs/index.ts` (scheduled Claude triage edge function) + `admin-cms/src/app/bug-intelligence/page.tsx` (triage inbox + PR button) + new admin API actions. Requires `ANTHROPIC_API_KEY` secret + `internal_config` rows.
- Phase 3 (this sprint): updates `supabase/functions/triage-bugs/index.ts` to (a) use `crash_reports.bi_fingerprint` for O(1) crash lookup, (b) include `stack_trace`, `breadcrumbs`, `session_log_snippet` in Claude input, (c) cross-correlate log-sourced fingerprints with crashes that share the same normalized_message (via `bug_intelligence_fingerprint()` RPC), and (d) instruct Claude in the system prompt to use the top Fit33-owned stack frame as `file_path`. The goal is for crash-sourced reports to almost always produce `file_path` + `code_diff` instead of null. Re-deploy with `supabase functions deploy triage-bugs` after migration 65.
