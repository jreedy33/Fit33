...
| PP-009 | Can't take progress photos | High | Feature shipped: `ProgressPhotoService.swift` + `ProgressPhotoViews.swift`, gallery linked from Profile | **Resolved (verified 2026-07-26)** |
...
| PP-016 | "Cardio gave me 0 LP, but my friend's gave 50" | High | Old client-side `+50 cardioSession` award was bypassed on `RunCompletionView` silent-skip path AND on Strava/HK imports. Net: native Fit33 runs sometimes credited, Strava-imported runs never did. | **Partially resolved / REOPENED (2026-07-26 audit)** — the `record_cardio_workout` RPC awards graduated LP for all sources, but the client-side `+50 .workout` was NOT removed from `UserManager.completeCardioWorkout` (only the `.cardioSession` award was). RPC-backed native cardio now DOUBLE-awards (+100 vs strength's +50). Tracked as MASTER_TODO PR-14. Expect the opposite complaint ("cardio gives more LP than lifting") until it lands. |
