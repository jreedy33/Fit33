...
3. **Cardio LP ownership — KNOWN OPEN BUG (MASTER_TODO PR-14, found 2026-07-26).** The intended split ("`+50 .workout` on the client, graduated `cardio_bonus` on the server") is NOT what ships: `record_cardio_workout` (migration #185) awards BOTH `add_league_points(...,50,'workout','cardio_native:…')` AND the graduated `cardio_bonus` (base + intensity_multiplier × km, capped +50/day), while `UserManager.completeCardioWorkout` ALSO unconditionally calls `addPoints(source: .workout)` — and the `workout` source has **no daily cap** (`league_point_source_caps.workout.daily_cap = NULL`). Net: native RPC-backed cardio earns **+100 workout LP** vs strength's +50. Resolution decision: the RPC is the single owner for RPC-backed saves — the client must skip `addPoints(.workout)` when the save went through `record_cardio_workout` (keep the client award only for genuinely non-RPC paths). Until PR-14 lands, do not "fix" either side in isolation. The legacy client-side `+50 .cardioSession` WAS correctly removed — re-introducing it triple-counts. Same-origin overlap dedup + cross-origin Strava merge happen inside the RPC; never replicate that logic on the client.
