...
struct Fit33WatchApp: App {
    ...
    /// Foreground UI state. Lives at App-scope so the views can be
    /// torn down + remounted without losing the last-known data.
    @StateObject private var todayStore = WatchTodayStore()
    @StateObject private var liveStore = WatchLiveWorkoutStore()
    ...
    var body: some Scene {
        WindowGroup {
            WatchContentView()
                ...
                .environmentObject(todayStore)
                .environmentObject(liveStore)
                .task {
                    Self.log.info("Fit33Watch foreground task — bootstrap")
                    await lifecycle.bootstrap()
                    // Wire the live-workout store into the WCSession
                    // bridge so phone-side push lands in the right
                    // ObservableObject. Idempotent.
                    WatchConnectivityBridge.shared.liveWorkoutStore = liveStore
                }
        }
    }
}
