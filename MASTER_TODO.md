...
| Q2-38-b | [ ] **OTP entry UI consolidation** — the two views still differ in their code-entry styling. Pick one canonical entry component (digit boxes or single field) and retire the other. Low-priority cosmetic follow-up. | `Fit33/ExistingUserPhonePrompt.swift`, `Fit33/PhoneVerificationSheet.swift` | Product Engineer + Design |
...
| Q2-41-b | [ ] **Migrate `ProgramExplorerView` to `SmartProgramRecommender`.** The last remaining legacy entry point (`ProgramLibraryService.getRecommendedPrograms`) returns `[WorkoutProgram]`; the canonical recommender returns `[ExtendedProgram]`. Write a mapper, swap the caller, delete the legacy method. | `Fit33/ProgramExplorerView.swift:36-44`, `Fit33/ProgramLibraryService.swift:270`, `Fit33/SmartProgramRecommender.swift` | Product Engineer + Fitness Expert |
