...
### 2026-04-20: Daily Quest Programming Rules (hard-won)

**Do NOT prescribe two full workouts in a single day as a daily goal.** The `complete_2_workouts` quest template is permanently retired (`is_active = FALSE`) and the hard-day fallback in `get_daily_quests` no longer contains it. Two training sessions/day is advanced-athlete programming (doubles, AM/PM splits for marathoners, sport-specific templates) — it is not appropriate as a one-size-fits-all daily goal, and most users physically cannot complete it. If the app ever wants to reward multi-session days, gate it behind `min_workouts >= 100` AND `current_streak >= 14` AND frame it as "activity combo" (e.g. strength + cardio or strength + walk), never two strength sessions.

**Quest difficulty must track workout experience.** `quest_templates.min_workouts` is the skill gate; the selector filters by `min_workouts <= p_total_workouts`. Current gates (see `DATA_BACKEND_AGENT.md` for the full list): 10 workouts to see `exercise_sets_25` / `hit_protein_goal` / `perfect_day`; 15 workouts to see `beat_personal_record` / `beat_volume_pr` / `league_3_workouts`; 50 workouts to see `top_3_league`. Any new hard quest MUST set `min_workouts` appropriately — 0 is only acceptable for low-stakes "move + log" behaviours.

**`requires_context` is mandatory on any quest that depends on app state.** The selector enforces it; a quest without the right `requires_context` tag will be handed out to users who cannot complete it (this was the "Complete your next program day" bug — it was missing no gate, the selector was silently ignoring the gate it had). Valid values: `has_program`, `has_friends`, `has_challenge`, `no_friends`, `no_challenge`, `free_user`, or `NULL` for "always eligible".
