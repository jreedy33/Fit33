...
30-cardio-2. **`supabase/20260815_record_cardio_workout_rpc.sql` (#185) ships `record_cardio_workout(p_payload JSONB) RETURNS JSONB`.** SECURITY DEFINER, `auth.uid()`-pinned (Data invariant #7 — payload's optional user_id is IGNORED). Behaviors: (a) idempotency on `(user_id, external_id)` — second call with the same external_id returns the existing row; (b) same-origin overlap dedup (existing row whose start..end window overlaps ≥50% on same `origin_app` becomes the survivor); (c) cross-origin Strava merge — when the incoming row is `fit33` and a Strava row covers the same window, the Strava row is kept and the incoming fit33 row is rejected as `merged_into_strava` (returns the Strava row's id); (d) graduated league points: `base + intensity_multiplier * km`, capped at +50 / day for `cardio_bonus`; the canonical `+50 .workout` LP stays in `UserManager` (parity with strength sessions); (e) PR detection writes to `cardio_personal_records`; (f) friend-feed insert into `friend_activity_feed` only when distance > 0 OR duration > 5min. Returns `{cardio_workout_id, was_inserted, league_points_awarded, cardio_bonus_awarded, prs_set, friend_feed_posted}`.
