-- ============================================================================
-- FIX: Only return TRULY active challenges (both users accepted)
-- Previously: returned challenges where creator accepted but opponent hasn't
-- Now: only returns challenges with status = 'active' (both accepted)
-- ============================================================================
...
BEGIN
    ...
    SELECT 
        ...
        -- Today's progress for current user
        COALESCE((
    ...
    -- FIX: Only return challenges where BOTH users have accepted (status = 'active')
    -- Previously this also included 'pending' which showed challenges before opponent accepted
...
-- Verify the fix
SELECT 'Fixed! Now get_active_challenges() only returns challenges where both users accepted.' as result;
