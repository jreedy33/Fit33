...
23-nuj-iserror. **`new_user_journey_events.is_error` is SEVERITY-driven, not eventType-driven (`Fit33/NewUserJourneyTracker.swift::recordEvent`, 2026-05-10).** The `Logger.swift` forwarder ALWAYS calls `NewUserJourneyTracker.logError(...)` for any `.warning+` log line, which sets `event_type = "error"` regardless of the underlying severity. Pre-fix, `recordEvent` then computed `is_error = (eventType == "error" || eventType == "crash") || (severity == "error" || severity == "critical")` — so a `CancellationError` classified as `.transientNetwork` by `NetworkErrorClassifier` (logged at `.warning`) was still flagged as `is_error = TRUE` in NUJ even though the bug-intel pipeline correctly classified it as transient. Net effect on the 2026-05-10 audit: 161 occurrences of "Failed to check achievement" inflated NUJ per-user error rates to 75%+ for some users. Fix: `is_error = eventType == "crash" || severity == "error" || severity == "critical"`. The `eventType == "error"` clause was the bug — it conflated "this came through the error-forwarding API" with "this is actually a real error." **Invariant**: the NUJ error rate is a PRODUCT FRICTION metric (drove the audit-PM call) — it MUST agree with `NetworkErrorClassifier`'s severity routing, not the AppLogger forwarding API shape. Future severity-aware rollups (per-screen, per-funnel-step) MUST also key off severity, not eventType.
