...
import { buildCorsHeaders, requireServiceOrAdminEmail } from "../_shared/cors.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    // This endpoint dumps PLATFORM-WIDE aggregates and invokes paid Anthropic
    // requests. Restrict to service-role OR an explicitly allowlisted admin
    // email (see ai_insights_admin_emails table).
    const authResult = await requireServiceOrAdminEmail(req, supabase, corsHeaders);
