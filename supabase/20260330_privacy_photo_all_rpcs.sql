-- Privacy Photo Enforcement: ALL remaining RPCs
-- Prerequisite: 20260330_privacy_settings.sql (privacy_hide_photo column must exist)
-- Patches RPCs NOT covered by 20260330_privacy_rpc_enforcement.sql:
--   1v1 challenges, group challenges, community challenges,
--   private challenges, shared workouts, friend requests.
-- Pattern: CASE WHEN COALESCE(up.privacy_hide_photo, FALSE) THEN NULL ELSE up.profile_photo_url END

BEGIN;

-- ============================================================================
-- 1. get_active_challenges (1v1) — hide opponent photo
-- ============================================================================
...
-- ============================================================================
-- 2. get_pending_sent_challenges — hide opponent photo
-- ============================================================================
...
BEGIN
    ...
    SELECT gc.id, gc.challenge_type, gc.title, gc.description,
        ...
        gc.daily_target, gc.total_target, gc.target_unit,
        gc.start_date::TEXT, gc.end_date::TEXT, gc.duration_days,
        opp.user_id, up.name, up.username,
...
-- ============================================================================
-- 3. get_active_group_challenges — hide member photos in JSON
-- ============================================================================
...
-- ============================================================================
-- 4. get_received_workouts — hide sender photo
-- ============================================================================
...
BEGIN
    ...
    IF current_user_uuid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
    RETURN QUERY
    SELECT sw.id, sw.sender_id, up.name, up.username,
        CASE WHEN COALESCE(up.privacy_hide_photo, FALSE) THEN NULL ELSE up.profile_photo_url END,
...
-- ============================================================================
-- 5. get_pending_friend_requests — hide requester photo
-- ============================================================================
...
BEGIN
    ...
    IF current_user_uuid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
    RETURN QUERY
    SELECT f.id, f.requester_id, up.name, up.username,
        CASE WHEN COALESCE(up.privacy_hide_photo, FALSE) THEN NULL ELSE up.profile_photo_url END,
...
-- ============================================================================
-- 6. get_sent_friend_requests — hide addressee photo
-- ============================================================================
...
BEGIN
    ...
    IF current_user_uuid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
    RETURN QUERY
    SELECT f.id, f.addressee_id, up.name, up.username,
        CASE WHEN COALESCE(up.privacy_hide_photo, FALSE) THEN NULL ELSE up.profile_photo_url END,
        f.status::TEXT, f.created_at, f.message,
        COALESCE(up.is_verified, FALSE),
        COALESCE(up.is_gold_verified, FALSE)
...
-- ============================================================================
-- 7. get_community_challenge_leaderboard — hide photos in leaderboard JSON
-- ============================================================================
...
-- ============================================================================
-- 8. get_my_community_challenges — hide photos in top_participants + friends_in JSON
-- ============================================================================
    ...
    SELECT cc.id, cc.title, cc.description, cc.emoji, cc.challenge_type,
        ...
        (SELECT COALESCE(jsonb_agg(entry ORDER BY (entry->>'rank')::INT), '[]'::JSONB) FROM (
            SELECT jsonb_build_object(
                ...
                'profile_photo_url', CASE WHEN COALESCE(sub_up.privacy_hide_photo, FALSE) THEN NULL ELSE sub_up.profile_photo_url END,
        ...
        (SELECT COALESCE(jsonb_agg(fi), '[]'::JSONB) FROM (
            SELECT jsonb_build_object('user_id', f_ccp.user_id, 'name', f_up.name,
                ...
                'profile_photo_url', CASE WHEN COALESCE(f_up.privacy_hide_photo, FALSE) THEN NULL ELSE f_up.profile_photo_url END) AS fi
...
-- ============================================================================
-- 9. get_community_challenge_detail — hide photos in friends_in + top_leaderboard JSON
-- ============================================================================
    ...
    SELECT cc.id, cc.title, cc.description, cc.emoji, cc.challenge_type,
        ...
        (SELECT COALESCE(jsonb_agg(entry ORDER BY (entry->>'rank')::INT), '[]'::JSONB) FROM (
            SELECT jsonb_build_object(
                ...
                'profile_photo_url', CASE WHEN COALESCE(up4.privacy_hide_photo, FALSE) THEN NULL ELSE up4.profile_photo_url END,
...
-- ============================================================================
-- 10. get_private_challenge_detail — hide photos in leaderboard JSON
-- ============================================================================
...
RETURNS TABLE (
    challenge_id UUID, title TEXT, description TEXT, emoji TEXT, cover_image_url TEXT, challenge_type TEXT,
    ...
    SELECT pc.id, pc.title, pc.description, pc.emoji, pc.cover_image_url, pc.challenge_type,
        ...
        (SELECT jsonb_agg(entry ORDER BY entry->>'rank') FROM (
            SELECT jsonb_build_object(
                ...
                'profile_photo_url', CASE WHEN COALESCE(up5.privacy_hide_photo, FALSE) THEN NULL ELSE up5.profile_photo_url END,
                'role', m5.role,
...
-- ============================================================================
-- 11. get_my_private_challenges — hide photos in top_members JSON
-- ============================================================================
...
RETURNS TABLE (
    challenge_id UUID, title TEXT, description TEXT, emoji TEXT, cover_image_url TEXT,
    challenge_type TEXT, daily_target INT, target_unit TEXT,
    member_count INT, max_members INT, join_code TEXT,
    is_recurring BOOLEAN, show_leaderboard BOOLEAN, allow_member_invites BOOLEAN,
...
-- ============================================================================
-- 12. get_friends — hide friend photo when they have privacy_hide_photo
-- ============================================================================
...
BEGIN
    ...
    SELECT f.id, CASE WHEN f.requester_id = current_user_uuid THEN f.addressee_id ELSE f.requester_id END,
        up.name, up.email, up.username, up.fitness_goal, up.experience_level,
        CASE WHEN COALESCE(up.privacy_hide_photo, FALSE) THEN NULL ELSE up.profile_photo_url END,
        f.created_at,
...
-- ============================================================================
-- 13. get_friends_in_community_challenge — hide friend photos
-- ============================================================================
...
RETURNS TABLE (
    user_id UUID, name TEXT, username TEXT, profile_photo_url TEXT,
    today_progress INT, days_completed INT, current_streak INT, best_streak INT,
    rank INT, is_direct_friend BOOLEAN
)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE current_user_uuid UUID; v_challenge_id UUID; today_date DATE;
BEGIN
    ...
    WITH ranked_participants AS (
        SELECT ccp.user_id, COALESCE(cdp.progress_value, 0) AS today_prog,
            ccp.days_completed, ccp.current_streak, ccp.best_streak,
            ROW_NUMBER() OVER (ORDER BY COALESCE(cdp.progress_value, 0) DESC, ccp.days_completed DESC)::INT AS rank
        FROM community_challenge_participants ccp
        LEFT JOIN community_challenge_daily_progress cdp ON cdp.challenge_id = ccp.challenge_id
            AND cdp.user_id = ccp.user_id AND cdp.progress_date = today_date
        WHERE ccp.challenge_id = v_challenge_id AND ccp.is_active = TRUE
    )
    SELECT rp.user_id, up.name, up.username,
        CASE WHEN COALESCE(up.privacy_hide_photo, FALSE) THEN NULL ELSE up.profile_photo_url END,
        rp.today_prog::INT, rp.days_completed::INT, rp.current_streak::INT, rp.best_streak::INT,
        rp.rank, are_friends(current_user_uuid, rp.user_id)
