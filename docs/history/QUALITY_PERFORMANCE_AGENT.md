...
8. **Repeating Timers must have a lifecycle** (Sprint 2, Q2-26): any `Timer.scheduledTimer(withTimeInterval:repeats: true)` MUST store the handle, invalidate on `UIApplication.willResignActive`/`didEnterBackground`, and restart on `didBecomeActive`. If the timer is in an `@MainActor` class, also release observers in `deinit`. Failure mode: leaks for the app lifetime AND wakes the CPU while the app is suspended.
9. **Offline writes must be queued, not logged** (Sprint 2, Q2-34): network writes called from user actions (`saveWorkoutToCloud`, social posts) must persist failures to a retry queue. Use `CloudSyncRetryQueue.shared` (file-backed JSON in Application Support) or create a parallel queue for the new surface — never "log error and drop". The queue drains from `Fit33App.onChange(of: scenePhase)` on foreground.
10. **No polling Timers alongside Realtime** (Sprint 2, Q2-27): a view that already subscribes to Supabase Realtime for a domain MUST NOT additionally run a `Timer` that refetches the same data. Pick one. `FriendsTabView` is the canonical example: Realtime + `.refreshable` only.
11. **Push flush on every social write** (Sprint 2, Q2-35): services that generate social notifications (`FriendService`, `ChallengeService`, `PrivateChallengeService`, `ActivityFeedService`, `CommunityChallengeService`, `CardioActiveWorkoutView`) MUST call `PushNotificationService.shared.flushPushNotificationQueue(triggeredBy:)` after the write succeeds.
12. **Notification type allowlist is the single source of truth** (Sprint 2, Q2-36): `NotificationManager.knownNotificationTypes` is the authoritative set of `type` strings the client routes. Any new server-side push or in-app notification MUST add its type to both the enum (if stable) and the allowlist in the same PR. `NotificationAllowlistTests` enforces the contract.
13. **Single HK observer owner** (Sprint 3, Q2-28): `BackgroundChallengeSyncService` is the ONLY owner of `HKObserverQuery`. `HealthKitManager` and other consumers listen to `.healthStepsDidUpdate` / `.externalWorkoutSynced` — never register their own observer queries. Adding a new HK sample type? Add it to `BackgroundChallengeSyncService.hkSampleTypesToObserve`, and post the right notification from `handleBackgroundHealthUpdate`.
14. **Video prefetch consults NetworkMonitor** (Sprint 3, Q2-30): every speculative `VideoPreloadManager` / `VideoStreamingService.prefetch*` path MUST early-return on `NetworkMonitor.shared.shouldAvoidBackgroundTraffic` (`isExpensive || isConstrained`). On-demand playback the user initiated is NEVER gated. New prefetch entry points go through `VideoStreamingService.shouldSkipBackgroundPrefetch(reason:)`.
15. **AVAudioSession is refcounted, NOT kept alive** (Sprint 3, Q2-31): never call `AVAudioSession.sharedInstance().setActive(true)` at service init. Use `VideoStreamingService.activateAudioSessionIfNeeded()` (lazy, NSLock-guarded) on first player creation and `deactivateAudioSessionIfActive()` (`.notifyOthersOnDeactivation`) when the player cache is drained or the app backgrounds. This stops the app from silently interrupting the user's music every launch.
16. **Decorative animations gate on BOTH `isLowPowerMode` AND `accessibilityReduceMotion`** (Sprint 3, Q2-32): any ambient animation (orbs, gradients, pulses, shimmer) must short-circuit when EITHER predicate is true. The canonical helper is `AnimatedOrbBackground.shouldDisableMotion`. Never ship a new decorative animation without both checks.
17. **View-local `Timer`s require store + invalidate** (Sprint 4, Q2-38): every `Timer.scheduledTimer(withTimeInterval:repeats:)` that lives on a SwiftUI view MUST store the handle in `@State Timer?` (for one-off views) or an `@StateObject ObservableObject` wrapper (for shared patterns). Host views MUST call `.invalidate()` from `.onDisappear`, and the start function MUST invalidate any prior timer before scheduling a new one (rapid-tap protection). The canonical shared helper is `PhoneOTPCountdown` (defined at the bottom of `Fit33/ExistingUserPhonePrompt.swift`); use it for any new countdown surface. Bare `Timer.scheduledTimer { ... }` with no stored reference is a bug and will be rejected in review. This rule supplements #8 — #8 covers service-lifetime timers, this covers view-lifetime timers.
...
- `syncSetsWithPreviousData()` must check if user has started typing before resizing — guard: only resize when ALL sets are empty (`isCompleted == false` AND `weight == 0` AND `reps == 0`). It only adjusts row count now; it never copies values into `setData` (see `DATA_BACKEND_AGENT.md` "Repeat-exercise placeholder contract" 2026-04-20).
...
- **Exercise Library never shows a loading / grey placeholder state.** On cold start and fresh install, `ExerciseLibraryService.preWarmCache()` runs a `Task.detached` on a background Core Data context and, if the Exercise table has <100 rows, inline-seeds all bundle JSON exercises inside the same `bgContext.perform` transaction BEFORE flipping `isExercisesReady = true`. `Fit33App.init()` touches `ExerciseLibraryService.shared` on its very first line so this pre-warm fires ASAP. The one-time `exercise_lever_fix_applied_v1` migration (in `Fit33App.init`) now also re-seeds from bundle inside its wipe transaction so post-migration Core Data is never empty. `ExerciseLibraryView` has no "Loading exercises..." ProgressView block, and `ExerciseNicknameService.displayName(for: Exercise)` returns `""` (not `"Exercise"` / `"Loading..."`) for a nil-name row — the library view filters empty-name rows out of the `ForEach` so a faulted/partial row never renders the grey placeholder card. NEVER re-introduce a skeleton/spinner path in the Exercise Library; if rows are missing, fix the data layer.
...
- HealthKit observers: **Sprint 3 (Q2-28)** removed `HealthKitManager.startObservingSteps()`/`startObservingWorkouts()` entirely. `BackgroundChallengeSyncService` is now the **single owner** of `HKObserverQuery` for steps, workouts, active energy, distance, and exercise time. After each sync it posts `.healthStepsDidUpdate` (steps/energy/distance/exercise) and `.externalWorkoutSynced` (workouts); `HealthKitManager` listens and refreshes its `@Published` UI state. Rule: **never register a new `HKObserverQuery` outside `BackgroundChallengeSyncService`**. Add new sample types to its `hkSampleTypesToObserve` set.
