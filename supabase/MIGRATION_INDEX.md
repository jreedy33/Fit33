...
| 75 | `20260509b_get_daily_quests_has_wearable_body.sql` | 🆕 Ready | (Phase 4 body patch) Recreates `get_daily_quests` with the full canonical body from migration 62 + ONE new parameter `p_has_connected_wearable BOOLEAN DEFAULT FALSE` appended + ONE new eligibility branch `OR (qt.requires_context = 'has_wearable' AND p_has_connected_wearable)` in the `_eligible_quests` CTE. Preserves the IDOR guard, layer 1/2/3 smart-hierarchy (Data invariant #30), redundancy matrix, challenge override (Data invariant #31), category diversity sweep, step-challenge copy rewrites, free-user ad slot — all verbatim. Idempotent DROP sweep of every historical overload first (supabase-rules §12). Unblocks the `AppConfig.FeatureFlags.wearableQuests = true` flag — once applied, users with WHOOP / Oura / Fitbit / HealthKit connected will start seeing the 6 new templates (sleep_8h_wearable, recovery_above_67, hrv_above_baseline, rhr_in_healthy_range, respect_red_recovery, log_readiness_am) in the daily rotation, subject to the same layer 1/2/3 rules. |
