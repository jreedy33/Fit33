-- ============================================================================
-- Migration: Fix get_daily_quests integer-overflow on abs(hashtext(...))
-- Date: 2026-04-25 (DailyQuestService 22003 "integer out of range")
-- Agent: Supabase / Data & Backend
--
-- Resolves: get_daily_quests PostgrestError 22003 "integer out of range"
--           reproduced on user 557F52B7… on 2026-04-25 (DailyQuestService
--           log line `[QUESTS] Failed to fetch: integer out of range`).
--
-- Why:
--   `20260605_get_daily_quests_personalized.sql` (release-train #106) builds
--   the per-day quest seed via:
--
--       v_day_seed := abs(hashtext(v_user_id::TEXT || v_today::TEXT));
--
--   `hashtext` returns `int4` (range -2147483648 … 2147483647). When the
--   hash happens to be exactly INT_MIN (-2147483648), `abs(int4)` overflows
--   because |INT_MIN| > INT_MAX — Postgres raises SQLSTATE 22003
--   ("integer out of range"). The bug is silent for most (user, date)
--   combos but deterministic for the unlucky ones — the user above is
--   stuck on the cached default goal slate every time they open the app
--   on 2026-04-25.
--
--   The same unsafe pattern repeats in 9 ORDER BY clauses inside the
--   function body —
--
--       ORDER BY selection_score DESC, (abs(hashtext(quest_key)::BIGINT) + v_day_seed) % 11
--
--   — which CAN ALSO trip 22003 even when the seed assignment was lucky,
--   because the addition `INT + INT` overflows when both operands are
--   near INT_MAX. (Today's failure is the seed line; the ORDER BY lines
--   are latent bombs that the next bug-intel sweep would surface.)
--
-- Fix (minimal):
--   1. Cast `hashtext(...)` to `BIGINT` BEFORE calling `abs()` everywhere
--      it appears. `abs(BIGINT)` cannot overflow for any int4 input
--      because BIGINT range is ±9.2 × 10^18.
--   2. Keep `v_day_seed INT` (no cascade changes to array subscript / mod
--      arithmetic) by collapsing the BIGINT seed back into the int4
--      range via `% 2147483647`.
--   3. The ORDER BY expressions stay BIGINT-typed inside ORDER BY (the
--      sort accepts any integer width) — only the abs() call site needs
--      the BIGINT cast.
--
-- All other behaviour (every layer, every param, the IDOR guard, the
-- 32-arg signature, GRANT EXECUTE, COMMENT) is preserved verbatim.
--
...
COMMENT ON FUNCTION get_daily_quests(
    ...
    'Smart Adaptive Daily Goals v3 (20260605, 22003-hardened 20260617): 6-layer personalized quest selection. Body identical to 20260605 except every abs(hashtext(...)) call now casts to BIGINT first to prevent SQLSTATE 22003 ("integer out of range") when hashtext returns INT_MIN. Layers — 1 workout slot, 2 redundancy matrix, 3 challenge override, 4 activity-mix bias + per-user weighting + suppression, 5 skip-streak floor, 6 friend-named copy + Pro 5-slot expansion.';
